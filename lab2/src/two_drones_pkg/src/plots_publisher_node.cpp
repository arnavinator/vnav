...
class PlotsPublisherNode{
 ros::Time startup_time;		// track timing
 ros::Timer heartbeat;			// set frequency of operation
 ros::NodeHandle nh;			// contact master, register node
 ros::Publisher markers_pub;		// publishes markers for rviz		
 tf::TransformListener tf_listener;	// listen to broadcast
 int num_trails;
 ...
 class TrajTrail{
   ...
   void update(){
     tf::StampedTransform transform; // transform to populate
     try {
          ...
          *       a small wait time.    
          */
         parent->tf_listener.waitForTransform("dest_frame", "ref_frame", ros::Time(0), ros::Duration(1.0));
         // populate StampedTransform "transform" with transform between dest and ref frame
         parent->tf_listener.lookupTransform("dest_frame", "ref_frame", ros::Time(0), transform);
...
int main(int argc, char** argv){
  ros::init(argc, argv, "plots_publisher_node");  // register the node
  PlotsPublisherNode node;
  ros::spin();   // a blocking call to ros::spin() is issued, which starts ROS’s main loop
  return 0;
}
