...
class FramesPublisherNode{
	// declare private nh, startup_time, heartbeat
 ros::NodeHandle nh;   					// contact master, register node
 ros::Time startup_time;    		// track timing
 ros::Timer heartbeat;					// determine frequency of onPublish
 tf::TransformBroadcaster br;		// broadcaster to send transforms to listener

 public:
  FramesPublisherNode(){  // __init__
  ...
  void onPublish(const ros::TimerEvent&){
    ...
    // for AV1, roll=pitch=0, yaw=t rotation, for AV2, identity rotation
    tf::Quaternion q;
		q.setRPY(0, 0, time);
...
int main(int argc, char** argv){
  ros::init(argc, argv, "frames_publisher_node");  // register the node
